...
const errHandeler = async (err, req, res, next) => {
    try {
        ...
        if (stack && statusCode !== 429) await db.collection("errLog").insertOne({ statusCode: statusCode, message: message, stack: stack, createdDate: isodate(new Date().toLocaleString("en-US", { timeZone: 'Asia/Kolkata' })), reqUser: objectify(user?.data[0]?._id) });
